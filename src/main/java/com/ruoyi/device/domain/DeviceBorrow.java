...
//    借用这信息（小程序）
    @Excels({
        @Excel(name="工号/学号",targetAttr = "regCode",type = Excel.Type.EXPORT),
        @Excel(name="姓名",targetAttr = "regName",type = Excel.Type.EXPORT),
        @Excel(name="部门/班级",targetAttr = "regDept",type = Excel.Type.EXPORT)
    })
    private DeviceUserRegister user;
//借用时间，开始时间，查询条件（从页面传递：可以为空）
    @JsonFormat(pattern = "yyyy-MM-dd")
    private Date beginBorrowTime;
//    借用时间：结束时间，查询条件（从页面传递：可以为空）
    @JsonFormat(pattern = "yyyy-MM-dd")
